...
**One-paragraph summary**: Zehner's 1972 packed-bed closure `Lambda/lambda = lambda_so/lambda + Pe/K` was validated with four independent methods (symbolic regression, gray-box ML + conformal UQ, differentiable JAX multiphysics, inverse PINNs) — it holds up, and every method breaks at exactly the same validity edges. Building on the ZBS-based electrode closure of Oehler (2021) -- which already includes the Knudsen correction and static contact -- this work supplies what such *forward* closures lack: a **process-resolved** contact term phi(Pi) that reproduces the measured u-shaped conductivity-calendering curve (MAPE 31.1% -> 4.5%), a **differentiable inverse** that reads coating porosity and interface/delamination quality from a thermal measurement, and **calibrated uncertainty**. Each non-forward capability is delivered by one of four data-driven methods (autodiff, GP+conformal, symbolic regression, PINN), and the framework transfers beyond batteries to fuel-cell gas-diffusion layers.
...
```bash
pip install -e .            # installs the closure as an importable package (zbs-electrode)
pip install -r requirements.txt   # exact pinned versions to reproduce the published results
```

After `pip install -e .`, `import electrode_thermal`, `import electrode_data`, etc. work from anywhere — no `sys.path` manipulation needed. The notebooks retain a `sys.path` fallback so they also run on a bare checkout.
